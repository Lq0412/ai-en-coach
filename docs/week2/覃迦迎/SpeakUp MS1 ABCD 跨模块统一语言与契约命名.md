...
| 代号 | 模块           | 一句话职责                                     | 拥有的核心对象                                                                                               |
| ---- | -------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| A    | `preparation`  | 定义练习前可选择的场景、背景和角色             | `ScenarioDefinition`、`ScenarioConfig`、`PreparationProfile`、`RoleDefinition`                               |
...
| D    | `review`       | 分析回答、生成证据反馈、管理重答请求和历史投影 | `TurnAnalysis`、`FeedbackItem`、`RetryRequest`、`HistoryRecord`                                              |
...
| `max_primary_questions`       | 主问题上限             |
| `max_follow_ups_per_question` | 单题追问上限           |
| `target_objectives`           | 本场需覆盖的目标       |
| `early_completion_rule`       | 允许提前完成的规则标识 |
...
| `owner_type`     | 所属对象类型；MS1 Review 链路使用 `TURN`     |
| `owner_id`       | 所属对象标识                                 |
| `duration_ms`    | 音频时长                                     |
...
### 6.5 Conversation 转录结构与 Review 只读快照

完整转录版本、Provider 和处理过程属于 Conversation 的内部数据。Conversation 通过 `TurnReviewSource` 向 Review 提供当前有效的 `TranscriptSnapshot` 只读快照，供评分和证据定位使用，但不暴露内部 Repository 和完整处理状态机。
...
Conversation 向 Review 提供的最小只读快照：

| 最小字段        | 含义                             |
| --------------- | -------------------------------- |
| `transcript_id` | 当前有效转录标识                 |
| `text`          | 当前有效回答文字                 |
| `language`      | 识别语言                         |
| `status`        | 转录是否已经可供复盘             |
| `segments`      | 带时间范围的必要片段集合，可为空 |

约束：

- Review 只能读取 `TranscriptSnapshot`，不能覆盖 Conversation 的原始 Transcript。
- Review 自行执行备用 ASR 时，产生的是 Review 内部的分析用转录，不回写 Conversation。
- `segments` 只传递证据定位需要的 `start_ms`、`end_ms` 和 `text`。

### 6.6 Conversation 向 Review 提供的复盘资料

Conversation 通过 `GetCompletedTurnReviewSource(turn_id)` 返回 `TurnReviewSource`。它是 Review 使用的最小只读快照，不是 Conversation 内部领域对象：

| 最小字段          | 含义                                      |
| ----------------- | ----------------------------------------- |
| `turn_id`         | 已完成 Turn 的稳定标识                    |
| `session_id`      | Turn 所属 Session                         |
| `question_id`     | 原问题标识                                |
| `question_text`   | 原问题文本，供评分时理解回答上下文        |
| `transcript`      | 当前有效的 `TranscriptSnapshot`           |
| `audio`           | 音频的 `AudioReference`，不含音频内容     |
| `completed_at`    | Turn 完成时间                             |

#### `AudioReference`

Conversation 保留完整 `AudioAsset` 所有权，向 Review 提供最小 `AudioReference`：

| 最小字段       | 含义                                           |
| -------------- | ---------------------------------------------- |
| `audio_id`     | 稳定音频引用，不是对象存储路径或公开下载地址 |
| `content_type` | 音频 MIME                                      |
| `format`       | 容器或编码格式                                 |
| `size_bytes`   | 音频大小                                       |
| `duration_ms`  | 音频时长                                       |
| `checksum`     | 完整性校验值，可为空                           |

Review 先取得 `AudioReference`，真正评分时再通过 `OpenAudio(audio_id)` 打开新的只读流。相同 `audio_id` 每次成功调用都应返回新的流，以支持失败重试；Review 用完后负责关闭流，不保存对象存储位置或凭证。

### 6.7 Conversation 内部失败与尝试结构
...
| `turn_id`               | 被分析的原始 Turn                  |
| `evaluator_version`     | 评分实现或 Mock 版本               |
| `analysis_status`       | 分析状态                           |
| `score`                 | 评分，可为空；具体范围暂不冻结     |
| `summary`               | 结构化结果的简要总结               |
| `analysis_transcript`   | Review 评分使用的转录，可为空      |
| `failure_reason`        | 失败原因，可为空                   |
| `created_at`            | 分析创建时间                       |
| `completed_at`          | 分析完成时间，可为空               |
| `failed_at`             | 分析失败时间，可为空               |
...
- 相同 `turn_id + evaluator_version` 只能存在一份有效分析。
- `completed` 必须包含评分结果和总结。
- `failed` 必须包含失败原因。
- Score 的范围和展示方式尚未冻结，不能只凭一个分数替代证据反馈。
- TurnAnalysis 状态不会反向修改来源 Turn 或 PracticeSession。
...
| `feedback_category` | 反馈维度，具体枚举由反馈契约冻结    |
| `message`          | 证据所反映的具体问题或有效表现       |
| `suggestion`       | 可执行的改进方向，可为空             |
| `evidence`         | 用户原回答中的具体证据集合           |
| `retryable`        | 是否允许根据该反馈发起同题重答       |
| `created_at`       | 反馈创建时间                         |

`Evidence` 支持文字、音频时间范围或二者同时定位：

| 最小字段          | 含义                                   |
| ----------------- | -------------------------------------- |
| `transcript_text` | 必要的原回答引用片段，可为空           |
| `start_ms`        | 音频证据开始时间，可为空               |
| `end_ms`          | 音频证据结束时间，可为空               |

证据必须至少包含文字或完整时间范围；不能复制完整原始 Transcript 充当证据。

### 7.3 `RetryRequest`

Review 创建重答请求，Conversation 根据该请求为原问题创建一个新 Turn。原 Turn、原分析和原反馈均不能被覆盖。

| 最小字段             | 含义                                       |
| -------------------- | ------------------------------------------ |
| `retry_request_id`   | 重答请求标识，同时作为跨模块幂等键         |
| `original_turn_id`   | 被重答的原始 Turn                          |
| `feedback_item_id`   | 触发重答的反馈                             |
| `new_turn_id`        | Conversation 创建的新 Turn，可为空         |
| `retry_status`       | 重答请求状态                               |
| `failure_reason`     | 创建新 Turn 失败的原因，可为空              |
| `created_at`         | 请求创建时间                               |
| `updated_at`         | 最近状态更新时间                           |
...
- `turn_created` 必须包含 `new_turn_id`。
- `failed` 必须包含 `failure_reason`。
- Conversation 对相同 `retry_request_id` 必须返回同一个 `new_turn_id`，不得重复创建 Turn。
- 新 Turn 是否计入 Session 主进度由 PracticeSessionPolicy 判断，Review 不直接修改 Session。

### 7.4 `HistoryRecord`

HistoryRecord 是 Review 维护的可重建只读投影，只服务于历史列表和复盘查询，不是 Session、Turn 或 Analysis 的第二份权威数据。

| 最小字段             | 含义                         |
| -------------------- | ---------------------------- |
| `history_record_id`  | 投影记录标识                 |
| `practice_session_id` | 来源 Session                |
| `turn_id`            | 来源 Turn                    |
| `turn_analysis_id`   | 来源 Analysis                |
| `retry_request_id`   | 关联重答请求，可为空         |
| `score`              | 历史展示用评分               |
| `summary`            | 历史展示用摘要               |
| `reviewed_at`        | 复盘完成时间                 |
...
- 使用 `practice_session_id + turn_id + turn_analysis_id` 幂等更新。
- HistoryRecord 只从 `completed` 的 TurnAnalysis 生成，必须包含评分和摘要。
- HistoryRecord 可以从来源 ID 重建，不能通过历史查询修改源对象。
- 列表默认按 `reviewed_at` 倒序返回。
...
| C → D      | `TurnReviewSource`（Question、`TranscriptSnapshot`、`AudioReference`） | 为已完成 Turn 生成分析和反馈 | D 只读，不修改 Question、Turn、转录或音频   |
| D → C      | `CreateRetryTurnCommand`                                              | 为原问题创建同题重答 Turn    | C 按 `retry_request_id` 幂等创建并返回新 Turn |
| D → 产品层 | `TurnAnalysis`、`FeedbackItem`、`RetryRequest`、`HistoryRecord`       | 展示评分、证据、重答和历史   | 产品层不能通过投影反向修改来源对象          |
...
A 创建/更新场景、背景和角色
  ...
  -> B 返回 NextAction
  -> D 从 C 获取已完成 Turn 的 TurnReviewSource
  -> D 生成并保存 TurnAnalysis、FeedbackItem 和 HistoryRecord
  -> 用户选择同题重答时，D 创建 RetryRequest
  -> D 请求 C 按 retry_request_id 幂等创建新 Turn
...
- 每场固定四个有效 Turn；无效输入不消耗轮次。
- 至少一次问题明确引用上一轮回答。
- 每个有效 Turn 至少产生一份确定性 Mock `TurnAnalysis` 和带证据的 `FeedbackItem`。
- D 通过 `RetryRequest` 发起同题重答，C 创建新 Turn；原 Turn、分析和反馈均不覆盖。
- D 提供基于 `HistoryRecord` 的历史列表和复盘查询。
...
| `Feedback` 泛指所有分析结果        | `TurnAnalysis` / `FeedbackItem`                   | 区分完整分析与单条证据反馈              |
| `RetryAttempt` 作为 D 的公共对象   | `RetryRequest` + Conversation 创建的新 `Turn`     | 区分重答请求与实际回答的所有权          |
| `SessionReport` 作为当前 D 核心对象 | `HistoryRecord`                                   | 当前阶段只维护可重建的历史只读投影      |
| `NextPracticeFocus` 作为当前 D 契约 | 后续单独评审                                      | 当前 Review 边界不负责下一场训练目标    |
| `History` 作为实体名               | `HistoryRecord` 或 `ListHistory`                  | 明确只读投影或查询，不使用模糊实体名    |
...
- [ ] C 确认 Question、Turn、`TranscriptSnapshot`、`AudioReference` 和轮次结果名称。
- [ ] D 确认 `TurnAnalysis`、`FeedbackItem`、`RetryRequest` 和 `HistoryRecord` 名称。
- [ ] C、D 确认 `TurnReviewSource`、`OpenAudio` 和 `CreateRetryTurn` 的契约及幂等语义。
- [ ] 四方确认其余跨模块传递清单和状态枚举。
