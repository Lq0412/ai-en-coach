...
1. 每条用户消息独立触发处理，不等待或合并后续消息，Agent 可以分 1—3 段回复；
2. 用户跨天回来时可以恢复同一真实事项；
...
用户每发送一条文字或语音消息，系统就立即处理这一条消息，不等待用户继续补充。信息不足时，Agent 可以先回应，再在后续对话中逐步补全场景。首次识别到明确面试任务后，Agent 创建一个 `Interview Scenario`：
...
![SpeakUp MVP 长期 Agent 总体架构](./attachments/2026-07-22-speakup-long-term-agent-architecture-v2.png)
...
| 输入处理层 | Message Ingest、ASR、Thread Queue | 单条消息接收、语音转写和线程内排队 |
| Agent Runtime | Run Manager、Context Builder、Bounded Agent Loop、Response Dispatcher、LLM Port | 上下文装配、模型循环、分段回复、预算和生命周期 |
...
- ASR、TTS、单条消息入站处理、Response Dispatcher 和默认 Context Builder 属于系统管线，不由模型自由选择；
...
## 6. 单条消息处理与 Agent 分段回复

### 6.1 MVP：一条用户消息触发一次处理

MVP 不等待用户连续发送多条消息，也不使用安静窗口或语义完整性分类器合并输入。用户每次点击发送，就形成一条独立的 `UserMessage`：

```text
文字消息：提交后立即进入 Thread Queue
语音消息：上传并完成 ASR 后进入 Thread Queue
```

每条 `UserMessage` 原则上触发一个 `AgentRun`。如果用户下一条消息是在补充前文，新的 Run 通过 Thread 最近消息和当前 Scenario 理解补充关系，而不是回头合并两条输入。
...
这样首版不需要实现多段语音聚合、静默等待和复杂的 `collect / steer` 语义，消息边界直接以用户的发送动作为准。

### 6.2 Agent 可以分段回复

一次 `AgentRun` 可以输出 1—3 个有顺序的 `ResponseSegment`，让用户更快看到第一段有效内容，而不是等待一篇长回复全部生成完成。

推荐的分段方式：

1. **直接回应**：先解决用户当前最明确的问题；
2. **补充内容**：给出解释、示例或检索到的依据；
3. **下一步动作**：需要时展示追问、材料请求或 ActionCard。

例如：

```text
第 1 段：可以，这是一场产品经理英文一面，我先帮你建立准备场景。
第 2 段：结合 JD，目前最值得准备的是项目贡献、跨团队协作和冲突处理。
第 3 段：你可以先发简历，或者直接讲一个最想练的项目。
```
...
ResponseSegment
  ...
  runId
  sequence
  type                      # text | example | evidence | action_card
  content
...
- 多个 Segment 属于同一个 `AgentRun`，不是多个 Agent 独立思考；
- 第一段必须有实际信息，不能只发送“好的”“我在处理”；
- 默认最多 3 段，避免连续刷屏；
- Segment 按顺序持久化和发送，失败后可以从未发送段重试；
- TTS 按 Segment 生成，用户开始说话时立即停止未播放音频；
- ActionCard 只能使用受控类型和固定路由。

### 6.3 Response Dispatcher

模型可以一次返回结构化 `segments[]`，也可以流式生成内容；统一由 `Response Dispatcher` 负责：

1. 校验 Segment 数量、类型和顺序；
2. 保存 Segment；
3. 通过 SSE / WebSocket 依次发送；
4. 为需要朗读的 Segment 触发 TTS；
5. 记录发送、取消和失败状态。

模型不能直接向客户端发送任意事件，避免出现乱序、重复消息或未经校验的页面动作。

### 6.4 新消息到达时的策略

同一 Thread 的可写 Run 保持串行，MVP 不合并两条用户消息：

| 当前状态 | 新消息到达 | MVP 策略 |
|---|---|---|
| 当前无 Run | 新 UserMessage | 立即创建 Run |
| Run 尚未发送任何 Segment | 新 UserMessage | 取消旧 Run，优先处理新消息 |
| 已发送部分 Segment | 新 UserMessage | 停止剩余 Segment 和 TTS，创建后续 Run |
| 已执行成功的写工具 | 新 UserMessage | 保留事务结果，并在新 Run 上下文中说明结果 |

已经发送给用户的 Segment 不撤回；尚未发送的 Segment 可以标记为 `cancelled`。用户的新消息始终作为新的 `UserMessage` 保存。

### 6.5 单次 AgentRun

每次 Run 执行：

1. 验证用户、`threadId` 和 `messageId`，创建 `runId`；
2. Context Builder 装配当前 UserMessage、会话记忆、当前 Scenario 和相关长期记忆；
3. Tool Policy 根据意图和权限暴露本轮工具；
4. 模型直接生成分段回复，或产生结构化 Tool Call；
5. Runtime 完成 Schema、权限、风险和幂等校验；
6. Tool Result 保存后进入下一次模型推理；
7. Response Dispatcher 按顺序发送并保存 Segment；
8. 保存 Run 结束状态和审计事件。
...
达到预算、工具要求补充信息、用户发来新消息、依赖不可用或安全策略拒绝时停止当前 Run，并取消尚未发送的 Segment。
...
- 不同用户、不同 Thread 可以并行；
- 多个只读查询可以并行，写工具默认串行；
- 先创建 Scenario 再搜索上下文时必须串行，因为搜索依赖 `scenarioId`；
...
每条完成转写或已提交的 `UserMessage` 都可以经过结构化字段抽取。Extractor 只生成带证据的字段候选，Domain Service 负责校验、冲突检测和合并；后续消息继续补充同一场景，不需要先把多条消息合并。
...
模型提供 `type`、`title`、`goal` 和已知字段；`userId`、`threadId`、`messageId`、`requestId` 由服务端注入。
...
response.segment.created / sent / cancelled / failed
...
5. **E2E Replay**：覆盖连续消息、分段回复中断、跨天恢复、工具超时和模型重试。
...
1. `AssistantThread / UserMessage / AgentRun / ResponseSegment / ToolCall`；
2. Scenario 数据模型和当前场景关联；
3. 单条消息触发、有界 Agent Loop 和 Response Dispatcher；
...
```text
Day 1
用户发送一条语音，说明下周有英文产品经理面试
→ ASR 完成后立即创建 AgentRun
→ 创建唯一 Interview Scenario
→ Agent 先发送简短有效回复，再分段补充需要的材料和下一步
→ 用户上传简历和 JD
→ 搜索岗位要求与相关项目经历
→ Agent 给出带来源的准备重点
...
验收重点：单条消息立即触发、分段回复不乱序且可中断、唯一创建、模糊意图不误创建、跨天正确找回、搜索不越权、结果有来源、上下文不超预算、整条链路可回放。
