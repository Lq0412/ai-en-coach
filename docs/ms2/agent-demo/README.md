...
├── live-agent/               # LiveKit Agent（Go STT/LLM/TTS 适配与事件桥）
├── .data/                    # 共享运行数据，不提交 Git
├── docs/                     # 交接与设计文档
└── package.json              # npm workspaces 与统一命令入口
```

前端、后端和 LiveKit Worker 是三个独立 workspace。日常仍从项目根目录运行
命令；需要单独操作时可用 `npm run <command> --workspace <workspace>`。
...
实时功能默认关闭。先在 `.env.local` 同时配置后端、前端和 Worker 的同一开关；
浏览器只能获得短期 room token，不能接触 LiveKit API Secret 或 DashScope Key：

```bash
export LIVEKIT_VOICE_ENABLED=true
export NEXT_PUBLIC_LIVEKIT_VOICE_ENABLED=true
export LIVEKIT_URL=wss://your-project.livekit.cloud
export LIVEKIT_API_KEY=your-livekit-api-key
export LIVEKIT_API_SECRET=your-livekit-api-secret
export LIVEKIT_TOKEN_TTL_SECONDS=600
export GO_BACKEND_URL=http://127.0.0.1:8080
export LIVE_AGENT_PROVIDER_TIMEOUT_MS=30000
```
...
有限恢复策略如下：LiveKit SDK 先做短暂网络重连；超出窗口后浏览器只调用一次
`resume` 刷新 token，并要求返回原 `live_session_id`。恢复仍失败时自动结束 room、
释放麦克风并回到普通聊天。页面从后台回到前台或网络恢复时只触发这一次受控恢复；
Go 的幂等键确保已经 canonical committed 的 turn 不会重复写入。LLM/TTS 使用
单轮 deadline，STT 使用会随音频和服务事件续期的 idle timeout。用户录音上传与
文本轮次解耦：上传失败会显示“录音未保存”，但不会撤销已成功的用户文字和 AI
回复。

关闭或回滚时，将 `LIVEKIT_VOICE_ENABLED` 和
`NEXT_PUBLIC_LIVEKIT_VOICE_ENABLED` 都设为 `false` 并重启三个进程。新页面不再
显示入口；已有页面在配置更新或卸载时收到结束状态并释放媒体资源。以下任一情况
应保持关闭：canonical 消息重复、麦克风未释放、连续恢复循环、关键延迟阶段无法
关联，或真实三轮对话/打断验收未通过。

隐私注意：实时音频会发送到 LiveKit，再由 Worker 转发给 Go/DashScope；用户录音
仅在上传和关联成功后保存。日志和 data event 只记录 session/turn/client message
关联 ID、阶段和经过脱敏的错误，不记录 token、API Key、Secret 或原始音频内容。
...
5. 面试不使用固定题库；每次把目标岗位、已问问题和全部有效回答交给模型，
   由模型生成贴合上一轮内容的真实追问。
6. 默认限制为 15 分钟但不固定题数；每一题结合目标岗位、候选人资料、历史问答和
   用户最新发言动态生成。时间到或用户主动结束时调用 `review.generate_feedback`；
   用户明确要求时仍支持 5–60 分钟时限和可选的 1–100 个有效回答安全上限。
...
自动化测试不等同于真实音频验收。发布前必须在真实 LiveKit 项目和浏览器麦克风
权限下手动完成：连续三轮实时对话、一次说话中打断、一次后台切回、一次断网恢复、
翻译、纠错、用户录音播放、结束后继续普通聊天；同时检查结构化 latency event
能够区分 ASR final、LLM first text、TTS first/stopped 与网络重连。当前仓库未
提供真实 LiveKit 凭证，因此这些项目是发布前人工 gate，不能用模拟结果替代。
