...
function panelMockPlan(){
  ...
  return alignNormalizePlan({id:'mock-panel-plan',mockData:true,interviewMode:'panel',createdAt:'今天 12:08',sampleMode:true,candidateSource:'示例资料',focusTags:['简历深挖','岗位能力','英语表达'],snapshot:{jobName:'后端开发工程师',jobDescription:'负责高并发交易服务、异步工作流和生产可观测性。',resumeName:'示例候选人简历',candidateProfile:ALIGN_SAMPLE_PROFILE,focusScope:'简历深挖、岗位能力、英语表达'},interviewers,sessions:interviewers.map(()=>[]),panelSessions:[inProgress,completed],status:'ready'});
}
...
function panelRoundsView(){
  ...
  return `<div class="align-page">${alignTop(plan.snapshot.jobName||'群面计划','home')}<div class="plan-head"><div><span class="align-kicker">群面计划</span><h1 class="align-title">${esc(plan.snapshot.jobName)}</h1></div><span class="sample-flag">${plan.sampleMode?'示例模式':'3 位面试官'}</span></div><div class="plan-meta"><span>${esc(plan.candidateSource)}</span><span>${esc((plan.focusTags||[]).join('、'))}</span><span>25 分钟</span></div><section class="panel-overview"><div class="panel-overview-copy"><span class="align-kicker">多对一模拟</span><h2>3 位面试官，同场轮流追问</h2><p>人力面试官负责开场与收尾，业务和技术面试官会根据你的回答交替提问。</p></div><div class="panel-member-row">${members.map((p,i)=>`<div class="panel-member-card">${characterAvatar(i)}<b>${esc(p.name)}</b><small>${esc(p.role)}</small></div>`).join('')}</div><div class="panel-agenda">${['开场与动机','简历深挖','技术取舍','可靠性','反问收尾'].map((x,i)=>`<span><i>${i+1}</i>${x}</span>`).join('')}</div><button class="align-btn primary block" data-action="panel-start-session">${recent?.status==='in_progress'?'继续上次群面':'开始群面模拟'} · 约 25 分钟</button></section>${state.contextSheet?`<div class="specialty-sheet"><div class="specialty-panel"><div class="topbar"><h2>本计划背景资料</h2><button class="ghost-icon" data-action="align-close-context">×</button></div><div class="detail-card"><h3>${esc(plan.snapshot.jobName)}</h3><p>${esc(plan.snapshot.jobDescription)}</p></div></div></div>`:''}</div>`;
}
...
function panelPractice(){
  ...
  const phaseCopy={connecting:['连接中','正在建立受保护的群面语音连接'],ai:[`${speaker.name} 正在提问`,'你开口后，当前面试官会自动停止播放'],user:['请回答','请用英文回答，其他面试官不会抢话'],processing:['正在处理回答','保存转录后，下一位面试官将继续提问']}[phase]||[`${speaker.name} 正在提问`,'请准备回答'];
  if(state.interviewChatOpen)return `<div class="practice panel-practice">${panelConversation(session)}</div>`;
  return `<div class="practice panel-session-scene"><header class="single-scene-head"><button class="single-scene-back" data-route="rounds" aria-label="返回面试计划">‹</button><h1>群面模拟</h1><span class="single-scene-spacer" aria-hidden="true"></span></header><main class="panel-scene-main"><div class="panel-scene-members" aria-label="本场面试官">${members.map((p,i)=>`<div class="panel-scene-member ${i===current.speaker?'active':''}">${characterAvatar(i)}<span><b>${esc(p.name)}</b><small>${esc(p.role)}</small></span></div>`).join('')}</div><section class="single-scene-person panel-scene-speaker">${characterAvatar(current.speaker,true)}<h2>${esc(speaker.name)}</h2><p>${esc(speaker.role)}</p></section><section class="single-scene-question panel-scene-question"><small>第 ${turn+1} / 6 题 · ${esc(current.label)}</small><p>${esc(current.text)}</p><em>${phaseCopy[0]}</em><div class="single-scene-wave" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div></section>${phase==='user'?`<textarea class="single-scene-transcript" data-answer-transcript aria-label="实时转录">${esc(answer)}</textarea>${state.answerError?`<div class="form-error">${esc(state.answerError)}</div>`:''}`:''}</main><footer class="single-scene-controls"><button class="single-scene-control" data-action="toggle-interview-chat" aria-label="查看对话"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5.5h14v10H9l-4 3v-13Z"/></svg><small>对话</small></button><button class="single-scene-primary ${phase==='user'?'recording':''}" data-action="ms1-answer" ${['connecting','processing'].includes(phase)?'disabled':''}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="3" width="6" height="11" rx="3"/><path d="M6.5 11.5a5.5 5.5 0 0 0 11 0M12 17v4M9 21h6"/></svg><small>${phase==='user'?'提交回答':'开口回答'}</small></button><button class="single-scene-control" data-action="end-early" aria-label="结束群面"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 7 10 10M17 7 7 17"/></svg><small>结束</small></button></footer></div>`;
}
...
function completeHistoryView(){
  ...
  const planCard=(plan,planIndex)=>{
    ...
    const states=rounds.map(roundState),completed=states.filter(item=>item.done).length,activeIndex=Math.max(0,states.findIndex(item=>item.active)>=0?states.findIndex(item=>item.active):states.findIndex(item=>!item.done)>=0?states.findIndex(item=>!item.done):rounds.length-1),expanded=state.historyExpandedPlanId===plan.id;
    ...
    const primaryState=states[activeIndex],primaryMode=completed===rounds.length?'plan':primaryState?.active?'continue':primaryState?.done?'report':'start',activeRound=rounds[activeIndex],primaryLabel=completed===rounds.length?'查看面试计划':`${primaryState?.active?'继续':'开始'}${activeRound?.name||'本轮面试'}`,progressLabel=completed===rounds.length?'已完成':`已完成 ${completed} / ${rounds.length} 轮`;
...
const usesDedicatedAgentVoiceHeader=()=>state.route==='agent-chat'&&state.agentVoiceState!=='idle'&&!window.SPEAKUP_REAL_AGENT_BRIDGE;
bottomNav=function(){
  if(usesDedicatedAgentVoiceHeader())return appDrawer();
...
const SCENE_CONFIGS={
  ...
  project:{id:'project',request:'我想练习和海外同事讨论项目进展。',title:'海外项目沟通',shortTitle:'项目沟通',partner:'Alex',role:'海外项目同事',voice:'沉稳男声',level:'职场英语',duration:'约 10 分钟',agentReply:'好的。我已经准备了一个海外项目同步场景，你会和同事 Alex 讨论进展、风险和下一步。',opening:'Could you walk me through the latest project update?',goal:'汇报项目进展、说明风险，并协调下一步安排。',questions:['Could you walk me through the latest project update?','What is the biggest risk affecting the delivery date?','What support do you need from our team?','Can we agree on the next milestone and owner?'],answers:['The core API integration is complete, and we are now testing the payment flow.','The biggest risk is the delayed data migration, which may affect final testing.','I need the data team to confirm the migration schedule by Wednesday.','Yes. I will own the integration test and share the result this Friday.'],labels:['同步进展','说明风险','协调支持','确认行动'],summaryTitle:'项目沟通已完成',summaryCopy:'你已完成进展同步、风险说明、资源协调和行动确认。下一次可以进一步练习更简洁地表达时间与责任人。'},
...
roleCreate=function(){
  ...
  return `<div class="scene-agent-page">${topbar('场景练习','agent-chat')}<main class="scene-agent-thread"><article class="scene-agent-turn"><img src="../assets/speakup-agent.png" alt=""><div><b>SpeakUp</b><p>告诉我你想练习的英语场景。一句话就可以，角色和练习目标交给我。</p></div></article>${ready?`<article class="scene-user-turn">${esc(config.request)}</article><article class="scene-agent-turn"><img src="../assets/speakup-agent.png" alt=""><div><b>SpeakUp</b><p>${esc(config.agentReply)}</p></div></article><section class="scene-agent-result"><small>场景已整理</small><h2>${esc(config.title)}</h2><p>${esc(config.partner)} · ${esc(config.level)} · ${esc(config.duration)}</p><button data-route="role-preview">查看场景</button></section>`:`<div class="scene-agent-suggestions">${suggestions.map(item=>`<button data-action="scene-agent-example" data-scene-id="${item.id}">${esc(item.request.replace(/^我想练习/,''))}</button>`).join('')}</div>`}</main><footer class="scene-agent-composer"><button class="scene-agent-add" data-action="agent-more" aria-label="更多输入方式">＋</button><button class="scene-agent-placeholder" data-action="scene-agent-example" data-scene-id="${config.id}">${ready?'继续补充要求':'描述想练的场景'}</button><button class="scene-agent-voice" data-action="scene-agent-example" data-scene-id="${config.id}" aria-label="用语音描述场景">${sceneVoiceIcon}</button></footer></div>`;
};
...
rolePreview=function(){const config=sceneConfig(null);return `<div class="scene-confirm-page scene-confirm-card-page">${topbar('场景确认','role-create')}<main class="scene-confirm-stage"><section class="scene-confirm-card"><header class="scene-card-head"><small>场景已准备好</small><h1>${esc(config.title)}</h1><p>${esc(config.level)} · ${esc(config.duration)}</p></header><div class="scene-card-person">${roleAvatar(true)}<small>你的对话伙伴</small><h2>${esc(config.partner)}</h2><p>${esc(config.role)} · ${esc(config.voice)}</p></div><div class="scene-card-opening"><small>${esc(config.partner)} 的开场白</small><p>“${esc(config.opening)}”</p></div><div class="scene-card-goal"><small>练习目标</small><p>${esc(config.goal)}</p></div><footer class="scene-card-actions"><button class="scene-confirm-secondary" data-action="restart-role">重新描述</button><button class="scene-confirm-primary" data-action="role-start-chat">开始练习</button></footer></section></main></div>`};
...
render=function(){
  ...
  S.classList.toggle('has-app-menu',['agent-chat','home','profile'].includes(state.route)&&!usesDedicatedAgentVoiceHeader());
