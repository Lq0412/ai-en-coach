...
function panelRoundsView(){
  ...
  return `<div class="align-page">${alignTop(plan.snapshot.jobName||'群面计划','home')}<div class="plan-head"><div><span class="align-kicker">群面计划</span><h1 class="align-title">${esc(plan.snapshot.jobName)}</h1></div><span class="sample-flag">3 位面试官</span></div><div class="plan-meta"><span>${esc(plan.candidateSource)}</span><span>${esc((plan.focusTags||[]).join('、'))}</span><span>25 分钟</span></div><section class="panel-overview"><div class="panel-overview-copy"><span class="align-kicker">多对一模拟</span><h2>3 位面试官，同场轮流追问</h2><p>人力面试官负责开场与收尾，业务和技术面试官会根据你的回答交替提问。</p></div><div class="panel-member-row">${members.map(p=>`<div class="panel-member-card"><span class="panel-member-initial">${esc(p.name.charAt(0))}</span><b>${esc(p.name)}</b><small>${esc(p.role)}</small></div>`).join('')}</div><div class="panel-agenda">${['开场与动机','简历深挖','技术取舍','可靠性','反问收尾'].map((x,i)=>`<span><i>${i+1}</i>${x}</span>`).join('')}</div><button class="align-btn primary block" data-action="panel-start-session">${recent?.status==='in_progress'?'继续上次群面':'开始群面模拟'} · 约 25 分钟</button></section>${state.contextSheet?`<div class="specialty-sheet"><div class="specialty-panel"><div class="topbar"><h2>本计划背景资料</h2><button class="ghost-icon" data-action="align-close-context">×</button></div><div class="detail-card"><h3>${esc(plan.snapshot.jobName)}</h3><p>${esc(plan.snapshot.jobDescription)}</p></div></div></div>`:''}</div>`;
}
...
function careerContextView(){
  return `<div class="career-context-page">${topbar('SpeakUp','agent-chat')}<main class="career-context-chat"><p class="career-context-date">周五 · 系统设计面试</p><article class="context-agent-turn"><span class="context-agent-mark">S</span><div><b>SpeakUp</b><p>结合你最近 6 次练习，这次不用再从自我介绍开始。我建议重点练“技术取舍与量化依据”。</p></div></article><article class="context-user-turn">为什么是这个重点？</article><article class="context-agent-turn"><span class="context-agent-mark">S</span><div><b>SpeakUp</b><p>你已经能先给结论，但连续 3 次在回答“为什么没选另一个方案”时，都缺少流量、可靠性或业务结果作为依据。</p><p>另外，你的时态错误已经从每轮 3 处降到 1 处，它不再是当前最需要解决的问题。</p><section class="context-evidence"><small>我参考了这些过往信息</small><dl><div><dt>目标岗位</dt><dd>后端开发工程师 · 全球团队</dd></div><div><dt>已经改善</dt><dd>回答结构、时态一致性</dd></div><div><dt>仍需补强</dt><dd>量化结果、替代方案</dd></div></dl></section></div></article><article class="context-user-turn">那下一轮怎么练？</article><article class="context-agent-turn"><span class="context-agent-mark">S</span><div><b>SpeakUp</b><p>我会让技术负责人沿着你的真实项目连续追问，并要求你说明选择、替代方案和结果证据。</p><section class="context-next-action"><small>下一轮练习</small><strong>系统设计 · 技术取舍深挖</strong><span>约 15 分钟 · 延续上次项目经历</span><button data-route="practice">按这个重点练</button></section></div></article><p class="career-context-control">SpeakUp 参考的过往信息由你掌控，可以随时查看、修改和删除。</p></main></div>`;
}
  ...
  return `<div class="agent-page agent-home agent-simple-home agent-idle">
    ...
    <div class="agent-quick-actions">
      <div class="quick-chips">
        ...
        <button data-action="agent-command" data-command="browse">浏览练习场景</button>
